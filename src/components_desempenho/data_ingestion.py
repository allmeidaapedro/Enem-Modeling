...
class DataIngestion:
    ...
    def apply_data_ingestion(self):
        ...
        try:
            logging.info('Read the dataset as a Pandas DataFrame and saving it as a csv.')

            path = '/Users/pedrohenriquealmeidaoliveira/Documents/data_science/Enem-Analysis/input/data/clean_df.parquet'
            df = pd.read_parquet(path)

            logging.info('Create the target and select the relevant observations.')
            ...
            # Get back train, test and validation entire sets.
            train = pd.concat([X_train, y_train], axis=1)
            test = pd.concat([X_test, y_test], axis=1)
            val = pd.concat([X_val, y_val], axis=1)

            logging.info('Save train, test and validation sets into a csv.')
