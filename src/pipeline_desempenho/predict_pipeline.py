...
class PredictPipeline:
    ...
    def predict(self, features):
        ...
        '''
        try:
            model_path = os.path.join('artifacts', 'model_desempenho.pkl')
            preprocessor_path = os.path.join('artifacts', 'preprocessor_desempenho.pkl')
            ...
            prepared_input_data = preprocessor.transform(features)

            logging.info('Predict.')
            
            # Predict candidate's grade on enem.
            predicted_grade = model.predict(prepared_input_data)

            # Prediction output (candidate's grande on enem).
            prediction = f'Nota predita = {round(predicted_grade[0], 2)}'
...
class InputData:
    ...
    def get_input_data_df(self):
        ...
        try:
            ...
            input_data_dict['estado_civil'] = [self.estado_civil]
